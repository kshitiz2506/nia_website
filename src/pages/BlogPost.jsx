...
function BodyParagraph({ text, index }) {
  ...
  return (
    <p {...revealProps} className="text-sm leading-relaxed text-white/70 sm:text-base">
      {text}
    </p>
  )
}
        ...
        <div className="mx-auto max-w-3xl px-4 lg:px-8">
          ...
          <div className="mt-10 space-y-6">
            {post.body.map((paragraph, index) => (
              <BodyParagraph key={index} text={paragraph} index={index % 4} />
            ))}
          </div>

          <aside
            {...ctaReveal.revealProps}
            className="mt-14 rounded-2xl bg-nia-card p-8"
          >
